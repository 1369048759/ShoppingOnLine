...
$DelayLoad.prototype = {
	...
	fnLoad: function(e) {
		...
		var owin = $(window),
			oLoad = othis.oLoadItems;
		var scrollStart = owin.scrollTop(),
			winH = owin.height(),
			end = scrollStart + winH;
...
$Slide.prototype = {
	...
	oInitMoveWay: {
		...
		moveWidth: function(e) {
			...
			othis.oPanel.css({
				"width": othis.itemValue * othis.sum
			});
			othis.setPos();
			othis.oPanel.css({
				"left": -cfg.pos * othis.itemValue
			});
			othis.setStatus();
		},
		...
		moveHeight: function(e) {
			...
			othis.oPanel.css({
				"height": othis.itemValue * othis.sum
			});
			othis.setPos();
			othis.oPanel.css({
				"top": -cfg.pos * othis.itemValue
			});
			othis.setStatus();
		},
		...
		moveHeightC: function(e) {
			...
			othis.oPanel.css({
				"width": othis.itemValue * (othis.sum + 1)
			});
			othis.setPos();
			othis.oPanel.css({
				"left": -cfg.pos * othis.itemValue
	...
	oMoveWays: {
		...
		moveHeightC: function() {
			...
			if (cfg.pos > othis.sum - 1) {
				othis.oItemsFirst.css({
					"left": othis.itemValue * othis.sum
				});
				othis.oPanel.animate({
					"left": -cfg.pos * othis.itemValue
				}, cfg.speed, function() {
	...
	fnDelay: function(e) {
		...
		if (spos > othis.oItems.length - 1) {
			spos = spos - othis.oItems.length;
		}
...
$SlideSimple.prototype = {
	...
	oInitMoveWay: {
		...
		moveWidthC: function(e) {
			...
			othis.itemValue = othis.oItemsFirst.outerWidth(true);
			othis.oPanel.css({
				"widthd": othis.itemValue * (othis.sum + 1)
			});
		},
		...
		moveHeightC: function(e) {
			...
			othis.oPanel.css({
				"height": othis.itemValue * (othis.sum + 1)
			});
		}
	},
	oMoveWays: {
...
function $addEvent(elem, type, handle) {
	...
	var typeArr = type.split("."),
		type = typeArr[0],
		spaceId = typeArr.length > 1 ? typeArr[1] : false;
...
function $removeEvent(elem, type, handle, delegateHandle) {
	...
	var typeArr = type.split("."),
		type = typeArr[0],
		spaceId = typeArr.length > 1 ? typeArr[1] : false;
	if (!elem.__eventHandle || !elem.__eventHandle[type]) {
		return;
	}
...
function $checkInfo() {
	...
	if (cookieValue === null || cookieValue.length == 0) {
		...
		$.ajax({
			url: "https://shop.gionee.com/user.php?act=get_login_user",
			dataType: "jsonp",
			data: {
				dtype: "jsonp",
...
function GetLoginInfoback(data) {
	...
	if ($userInfo.status == 0) {
		var exp = new Date();
		exp.setMinutes(exp.getMinutes() - 1000);
		document.cookie = 'gn_is_login=;expires=' + exp.toGMTString() + ';path=/;domain=.gionee.com;';
	} else {
...
function $getCartInfo(callback) {
	...
	$.ajax({
		url: "https://shop.gionee.com/flow.php?step=get_cart_goods",
		dataType: "jsonp",
		data: {
			dtype: "jsonp",
			jsoncallback: "callback",
...
$SlideOrgan.prototype = {
	...
	initStatus: function() {
		...
		for (var i = 0, len = that.itemsLen; i < len; i++) {
			that.domItems.eq(i).css({
				"zIndex": opt.zIndex + (len - i),
				"right": (len - i - 1) * that.navValue
			});
		}
		that.setStatus();
	},
	...
	movefunc: function() {
		...
		var len = that.itemsLen,
			itemValue = that.itemValue,
			navValue = that.navValue;
		that.setStatus();
...
$SlideReview.prototype = {
	...
	init: function() {
		...
		if (that.itemsLen === 2) {
			...
			that.domSlide.css({
				"overflow": "hidden",
				"width": that.domItems.eq(0).width(),
				"margin-left": "auto",
				"margin-right": "auto",
				"margin-top": "0",
				"margin-bottom": "0",
	...
	initStatus: function() {
		...
		that.domPanel.css({
			"width": that.itemValue * that.sumLen
		});
		that.setPos();
		that.domPanel.css({
			"left": -opt.pos * that.itemValue
		});
		that.setStatus();
	},
	setPos: function() {
